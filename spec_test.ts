...
/*
    Tests taken from Section 3.8.5.3 of the RFC (5545)
*/
...
Deno.test("Daily until December 24, 1997",
    () => {    
        ...
        assertEquals(previousDate.getFullYear(), 1997);
        assertEquals(previousDate.getMonth(), 11);
        assertEquals(previousDate.getDate(), 23);
        assertEquals(count, 113, "Number of days is wrong");
    }
);
...
Deno.test("Every 10 days, 5 occurrences",
    () => {  
        ...
        for (const eventDate of dateGenerator) {
            ...
            const diffDate = new Date(eventDate);
            diffDate.setDate(diffDate.getDate()-10);
            assertEquals(previousDate.getDate(), diffDate.getDate(), "Dates do not differ by 10 days");
            previousDate = eventDate;
            count++;
        }
...
Deno.test("Every day in January, for 3 years (as Daily)",
    () => {   
        const re = new RecurrenceRule("FREQ=DAILY;UNTIL=2000-01-31T15:00:00Z;BYMONTH=1", start);
        const dateGenerator = re.GenerateDate();
...
Deno.test("Weekly until December 24, 1997",
    () => {   
        ...
        //const re = new RecurrenceRule("FREQ=WEEKLY;UNTIL=1997-12-24Z", "1997-09-02T09:00:00Z");
        const dateGenerator = re.GenerateDate();
        ...
        assertEquals(previousDate.toString(), (new Date("1997-12-23T09:00:00-06:00")).toString(), "Last events don't match.");
        //assertEquals(previousDate.toString(), (new Date("1997-12-23T09:00:00Z")).toString(), "Last events don't match.");
...
Deno.test("Every other week - forever",
    () => {   
        ...
        for (const eventDate of dateGenerator) {
            const diffDate = new Date(eventDate);
            diffDate.setDate(diffDate.getDate()-14);
            assertEquals(previousDate.getDate(), diffDate.getDate(), "Dates do not differ by 14 days");
        ...
        console.log("previousDate: ", previousDate);
        assertEquals(previousDate.toString(), (new Date("1998-02-17T09:00:00-06:00")).toString(), "Last events don't match.");
        //assertEquals(previousDate.toString(), (new Date("1998-02-17T15:00:00Z")).toString(), "Last events don't match.");
    }
);
...
Deno.test("Weekly on Tuesday and Thursday for five weeks",
    () => {   
        ...
        console.log("previousDate: ", previousDate);
        assertEquals(previousDate.valueOf(), (new Date("1997-10-02T09:00:00-05:00")).valueOf(), "Last events don't match.");
...
Deno.test("Every other week on MWF until Dec 24, 1997, starting on Monday, Sept 1, 1997",
    () => {   
        const re = new RecurrenceRule("FREQ=WEEKLY;INTERVAL=2;UNTIL=1997-12-24Z;WKST=SU;BYDAY=MO,WE,FR", "1997-09-01T09:00:00-05:00");
        const dateGenerator = re.GenerateDate();
        ...
        //console.log("previousDate: ", previousDate);
        assertEquals(previousDate.valueOf(), (new Date("1997-12-22T09:00:00-06:00")).valueOf(), "Last events don't match.");
...
Deno.test("Every other week on Tu and Th, for 8 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1997-10-16T09:00:00-05:00")).valueOf(), "Last events don't match.");
...
Deno.test("Monthly on the first Friday for 10 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1998-06-05T09:00:00-05:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 10, "Expected event counts don't match.");  
    }
);
...
Deno.test("Monthly on the first Friday until December 24, 1997",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1997-12-05T09:00:00-06:00")).valueOf(), "Last events don't match.");
...
Deno.test("Every other month on the first and last Sunday of the month for 10 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1998-05-31T09:00:00-05:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 10, "Expected event counts don't match.");  
    }
);
...
Deno.test("Monthly on the second-to-last Monday of the month for 6 months",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getDay(), 1, "Date is not Monday");
            //console.log("RESULT: ", eventDate.toLocaleString());
            previousDate = eventDate;
            count++;
        }
        assertEquals(previousDate.valueOf(), (new Date("1998-02-16T09:00:00-06:00")).valueOf(), "Last events don't match.");
...
Deno.test("Monthly on the third-to-the-last day of the month, forever",
    () => {   
        const re = new RecurrenceRule("FREQ=MONTHLY;BYMONTHDAY=-3;COUNT=12", start);
        const dateGenerator = re.GenerateDate();
        ...
        assertEquals(previousDate.valueOf(), (new Date("1998-02-26T09:00:00-06:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 6, "Expected event counts don't match.");  
    }
);
...
Deno.test("Monthly on the 2nd and 15th of the month for 10 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1998-01-15T09:00:00-06:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 10, "Expected event counts don't match.");  
    }
);
...
Deno.test("Monthly on the first and last day of the month for 10 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1998-02-01T09:00:00-06:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 10, "Expected event counts don't match.");  
    }
);
...
Deno.test("Every 18 months on the 10th thru 15th of the month for 10 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1999-03-13T09:00:00-06:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 10, "Expected event counts don't match.");  
    }
);
...
Deno.test("Every Tuesday, every other month, forever",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDay(), 2, "Date is not Tuesday");
            previousDate = eventDate;
            count++;
            if (count == 18) break;
        }
        assertEquals(previousDate.valueOf(), (new Date("1998-03-31T09:00:00-06:00")).valueOf(), "Last events don't match.");
...
Deno.test("Yearly in June and July for 10 occurrences",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDate(), 10, "Date is not the 10th");            
            assertArrayIncludes([5,6], [eventDate.getUTCMonth()], "Month is not June or July");
            previousDate = eventDate;
            count++;
        }
...
Deno.test("Every other year on January, February, and March for 10 occurrences",
    () => {   
        ...
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDate(), 10, "Date is not the 10th");
            assertArrayIncludes([0,1,2], [eventDate.getUTCMonth()], "Month is not Jan, Feb or Mar");
            previousDate = eventDate;
            count++;
        }
...
Deno.test("Every Thursday in March, forever",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDay(), 4, "Day is not Thursday");
            previousDate = eventDate;
            count++;
            if (count == 11) break;
        }
...
Deno.test("Every Thursday, but only during June, July, and August, forever",
    () => {   
        ...
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDay(), 4, "Day is not Thursday");
            assertArrayIncludes([5,6,7], [eventDate.getUTCMonth()], "Month is not Jun, Jul or Aug");
            previousDate = eventDate;
            count++;
            if (count == 39) break;
        }
...
Deno.test("Every Friday the 13th, forever",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDay(), 5, "Day is not Friday");
            assertEquals(eventDate.getUTCDate(), 13, "Date is not 13th");
            previousDate = eventDate;
            count++;
            if (count == 5) break;
        }
        assertEquals(previousDate.valueOf(), (new Date("2000-10-13T09:00:00-05:00")).valueOf(), "Last events don't match.");
...
Deno.test("The first Saturday that follows the first Sunday of the month, forever",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDay(), 6, "Day is not Saturday");
            previousDate = eventDate;
            count++;
            if (count == 10) break;
        }
...
Deno.test("Every 4 years, the first Tuesday after a Monday in November, forever",
    () => {   
        ...
        let count = 1;
        for (const eventDate of dateGenerator) {
            assertEquals(eventDate.getUTCDay(), 2, "Day is not Tuesday");
            assertEquals(eventDate.getUTCMonth(), 10, "Month is not November");
            previousDate = eventDate;
            count++;
            if (count == 3) break;
        }
...
Deno.test("The third instance into the month of one of Tuesday, Wednesday, or Thursday, for the next 3 months",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1997-11-06T09:00:00-06:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 3, "Expected event counts don't match.");  
    }
);
...
Deno.test("The second-to-last weekday of the month",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1998-03-30T09:00:00-05:00")).valueOf(), "Last events don't match.");
...
Deno.test("Every hour and a half for 4 occurrences",
    () => {   
        ...
        assertEquals(previousDate.valueOf(), (new Date("1997-09-02T13:30:00-05:00")).valueOf(), "Last events don't match.");
        assertEquals(count, 4, "Expected event counts don't match.");  
    }
);
